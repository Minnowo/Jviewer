...
import nyaa.alice.jviewer.ui.enums.ZoomType;
        ...
        implements MouseListener, MouseMotionListener, MouseWheelListener, ActionListener, ComponentListener
{
     ...
     */
    public static final int[] ZOOM_PERCENT_DEFAULT = { 4, 7, 10, 15, 20, 25, 30, 50, 70, 100, 150, 200, 300, 400, 500,
            600, 700, 800, 1200, 1600, 2000, 2400, 2800, 3200, 3600, 4000, 4400, 4800, 5200, 5600, 6000, 6400, 6800,
            7200, 7600, 8000, 8400, 8800, 9200, 9600, 10000, 10400, 10800, 11200, 11600, 12000, 12400, 12800, 13200,
            13600, 14000, 14400, 14800, 15200, 15600, 16000, 16400, 16800, 17200, 17600, 18000, 18400, 18800, 19200,
            19600, 20000 };
    ...
    /**
     * the interpolation mode to use. determines the
     * RenderHints.VALUE_INTERPOLATION_xyz value
     */
    private byte interpolationMode = InterpolationMode.NEAREST_NEIGHBOR;

    /**
     * RenderingHints.VALUE_INTERPOLATION_xyz Object reference, this is changed with
     * interpolationMode
     */
    private Object interpolationModeObject = RenderingHints.VALUE_INTERPOLATION_NEAREST_NEIGHBOR;

    /**
     * the antialiasing mode to use. determines the RenderHints.VALUE_ANTIALIAS_xyz
     * value
     */
    private byte antiAliasing = AntiAliasing.DISABLED;

    /**
     * RenderingHints.VALUE_ANTIALIAS_xyz Object reference, this is changed with
     * antiAliasing
     */
    private Object antiAliasingObject = RenderingHints.VALUE_ANTIALIAS_OFF;

    /**
     * the render quality to use. determines the RenderHints.VALUE_RENDER_xyz value
     */
    private byte renderQuality = RenderQuality.QUALITY;

    /**
     * RenderingHints.VALUE_RENDER_xyz Object reference, this is changed with
     * renderQuality
     */
    ...
    /**
     * should the image be centered when first drawn, or when the drawMode requires
     * it
     */
    ...
    /**
     * used to check if the component has loaded with an image at least once this is
     * used to properly center the image on initial loads where the control might be
     * hidden
     */
     ...
     * 
     * @param zoomPercent           the int[] of zoomLevelPercent to use, THIS
     *                              SHOULD BE SORTED
    ...
    private class Selection
    {
         ...
         */
        private BasicStroke dashStroke = new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0.0f,
                new float[] { 8.0f, 8.0f }, 0);
         ...
         * 
         * it returns true if the given point is in a drag handle, and sets the
         * expanding to the given drag handle<br>
         * 
        ...
        /**
         * resets the expanding value to INVALID, used to cancel resizing of selection
         * via drag handles
         */
        ...
        {
            this.isInvalid = this.x1 == this.x2 || this.y1 == this.y2;
        ...
        public void render(Graphics g, Graphics2D g2)
        {
            ...
            // renders the outer area which makes a nice box with defined corners, this
            // visualizes all the drag handles
    ...
    /**
     * because the setImage function tries to center new images if this control
     * hasn't been shown yet it doesn't center correctly so this method can be used
     * by parent components to tell the control to recenter the image for initial
     * display
     */
...
//        }
    ...
    public BufferedImage getSelectionImage()
    {
        ...
        // dividing visible components by the zoom factor gets their 'actual' size, at
        // least with respect to the image
        // so find the difference between the top left selection and top left of the
        // image (visibly)
        ...
        BufferedImage newImage = new BufferedImage(Math.abs((int) (this.selectionArea.getWidth() / _zoom)),
     ...
     * this is a destructive change, it will fill the rest with transparency if
     * needed
     * 
     * @param degree
     */
...
//                }
    ...
    /**
     * sets the current zoom, assumes a percentage < MAX_ZOOM_PERCENT and >
     * MIN_ZOOM_PERCENT
     * 
    ...
    {
        if (value > MAX_ZOOM_PERCENT || value < MIN_ZOOM_PERCENT)
            return;
    ...
    protected Rect GetImageViewPort()
    {
        ...
        case ImageDrawMode.ACTUAL_SIZE:
            ...
            // since ACTUAL_SIZE mode only lets you drag the image if it's width / height >
    ...
    /**
     * adjust the x and y position of the image so the zoom appears to be onto the
     * mouse location
     * 
    ...
    private void ZoomIntoMouse(double beforeZoom, double nowZoom, Point mousePosition)
    {
        ...
        // it for the most part keeps the selection in the same spot, but moves it
        // slightly
        // would be nice to get it perfect but i'm not sure what i'd need to change for
        // that to happen
    ...
    /**
     * adjust the x and y position of the image so the mouse is centered on the
     * image
     * 
    ...
    /**
     * adjust the x and y position of the image so the zoom appears to be into the
     * bottom right corner
     * 
    ...
    /**
     * adjust the x and y position of the image so the zoom appear to be into the
     * center of the image
     * 
     * @param beforeZoom the zoom value before it changed
     * @param afterZoom  the new zoom value
    ...
    /**
     * find the nearest zoom in the {@link this.ZOOM_PERCENT} array and returns it's
     * index
     * 
    ...
    {
        ImageDisplayImageSizeChangedEvent event = new ImageDisplayImageSizeChangedEvent(newWidth, newheight, oldWidth,
                oldHeight);
    ...
    public void mousePressed(MouseEvent e)
    {
        if (e.getButton() == this.mouseDragButton)
        {
            ...
            // this will then be used in the mouse dragged function to highjack the image
            // drag and handle the selection instead
            if (this.allowSelection && !this.selectionArea.isInvalid
    ...
    {
        // always make sure to reset this, otherwise anytime the drag button is down,
        // its always changing the selection
        this.selectionArea.resetChanging();
    ...
    public void mouseDragged(MouseEvent e)
    {
        ...
        {
            if (!this.selectionArea.isInvalid && this.selectionArea.isExpanding()
                    && this.selectionArea.adjustSizeOrDie(e.getPoint()))
            {
                this.repaint();
                return;
            ...
            // actual size mode lets you drag the image if it's bigger than the control
            // wdith / height
    ...
    public void setVisible(boolean makeVisible)
    {
        ...
        // for some reason using any of https://stackoverflow.com/a/10881669 solutions
        // didn't work

...
public class IOHelper 
{
	...
	public static File getTempFilePath(String startingWidth, String endingWith, File directory)
	{
		...
		{
			f = new File(
					String.format("%s%s%s.%d.%s", directory.getAbsolutePath(), File.separatorChar, startingWidth, System.currentTimeMillis(), endingWith)
					);
		}
		while(f.exists());
	...
	public static void openFileExplorerToPath(File path) 
	{
		...
		{
			LoggerWrapper.warning(String.format("Failed to open %s in a file explorer", path), e);
		}
	}
}
